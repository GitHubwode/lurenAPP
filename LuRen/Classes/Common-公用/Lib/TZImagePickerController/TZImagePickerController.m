...
//    self.navigationBar.barStyle = UIBarStyleBlack;
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = LRROrangeThemeColor;
...
- (instancetype)initWithMaxImagesCount:(NSInteger)maxImagesCount columnNumber:(NSInteger)columnNumber delegate:(id<TZImagePickerControllerDelegate>)delegate pushPhotoPickerVc:(BOOL)pushPhotoPickerVc {
    ...
    if (self) {
        ...
        if (@available(iOS 8.2, *)) {
            self.barItemTextFont = LRRLightFont(15);
        } else {
            // Fallback on earlier versions
        }
        self.barItemTextColor = LRROrangeThemeColor;
...
- (instancetype)initWithSelectedAssets:(NSMutableArray *)selectedAssets selectedPhotos:(NSMutableArray *)selectedPhotos index:(NSInteger)index{
    ...
    if (self) {
        ...
        self.barItemTextColor = LRRTitleTextColor;
        [self configDefaultImageName];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (self.childViewControllers.count > 0) { // 如果viewController不是最早push进来的子控制器
        ...
        if (@available(iOS 8.2, *)) {
            backButton.titleLabel.font = LRRLightFont(15);
        } else {
            // Fallback on earlier versions
        }
        [backButton setTitleColor:LRRTitleTextColor forState:UIControlStateNormal];
